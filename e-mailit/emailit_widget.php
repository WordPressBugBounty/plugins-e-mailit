...
function emailit_admin_styles() {
    ...
    wp_enqueue_style('colorpickerCSS', plugins_url('js/colorpicker/css/spectrum.min.css', __FILE__));
    wp_enqueue_style('styleCSS', plugins_url('css/style.css', __FILE__));
    wp_enqueue_style('switchCSS', plugins_url('js/toggleswitch/css/toggleswitch.css', __FILE__));
}

function emailit_admin_scripts() {
    wp_enqueue_script('jquery');
    wp_enqueue_script('jquery-ui.minScript', plugins_url('js/jquery-ui.min.js', __FILE__));
    wp_enqueue_script('colorpickerScript', plugins_url('js/colorpicker/js/spectrum.min.js', __FILE__));
    wp_enqueue_script('fontselectScript', plugins_url('js/fontselect/jquery.fontselector.js', __FILE__));
    wp_enqueue_script('switchScript', plugins_url('js/toggleswitch/js/toggleswitch.js', __FILE__));
}
...
function emailit_admin_notices() {
    ...
    if (!get_user_meta($user_id, 'emailit_ignore_notice')) {
        ...
        printf(__('E-MAILiT Update: Mobile Share Bar | The most popular solution for your website. <a href="http://blog.e-mailit.com/2017/02/update-mobile-share-bar-most-popular.html" target="_blank">Check out the latest blog post.</a>. | <a href="%1$s">Hide Notice</a>', 'e-mailit'), '?' . http_build_query(array_merge($params, array('emailit_nag_ignore' => '0'))));
        echo "</p></div>";
    }
}
...
function add_script() {
    ...
    if ($emailit_options["FB_appId"] != "") {
        $configValues[] = "FB_appId:'" . $emailit_options["FB_appId"] . "'";
    }
    ...
    if ($emailit_options['mobile_position'] == "top") {
    ...
    $headline = array();
    if ($emailit_options['headline_content'] != "") {
        $headline['content'] = $emailit_options['headline_content'];
        $headline['font-family'] = $emailit_options['headline_font-family'];
        $headline['font-size'] = $emailit_options['headline_font-size'];
        $headline['color'] = $emailit_options['headline_color'];
    }
